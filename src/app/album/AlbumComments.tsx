...
export function AlbumComments({ albumId }: { albumId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    await supabase.from("album_comments").insert([{ album_id: albumId, author, password, content }]);
  ...
  const handleDelete = async (commentId: string) => {
    ...
    const { data, error } = await supabase.rpc("delete_album_comment_with_password", {
  ...
  return (
    <div className="mt-8 pt-8 border-t border-line-gray">
      ...
      <form onSubmit={handleSubmit} className="bg-paper-cream p-5 rounded-lg border border-line-gray">
        ...
        <textarea 
          placeholder="사진에 대한 생각과 나눔을 적어주세요." 
          value={content}
