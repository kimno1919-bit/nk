...
export function NoticeComments({ noticeId }: { noticeId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    await supabase.from("notice_comments").insert([{ notice_id: noticeId, author, password, content }]);
  ...
  const handleDelete = async (commentId: string) => {
    ...
    const { data, error } = await supabase.rpc("delete_notice_comment_with_password", {
  ...
  return (
    <div className="mt-8 pt-8 border-t border-line-gray/50">
